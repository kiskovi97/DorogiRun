...
{
    public MapGenerator(MapValues mapValues, float angle, float resolution)
    {
        ...
        for (int i=0; i < mapValues.sectorNumber; i++)
        {
            ...
            meshElements.Add(new SectorMesh(currentLeft, currentRight, mapValues.curve, angle, resolution));
        }
    }
}